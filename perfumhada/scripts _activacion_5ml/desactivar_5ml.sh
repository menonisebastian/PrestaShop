#!/bin/bash
# ============================================================
# DESACTIVAR combinaciones 5ml - Monoesencias y Esencias Parfum
...
echo ">>> Desactivando combinaciones de 5ml..."

mariadb -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" -e "
-- 1. Desactivar combinaciones de 5ml (atributo id=6) en categorías 82 y 114
...
SET pa.available_for_order = 0
WHERE pac.id_attribute = 6
AND cp.id_category IN (82, 114);

-- 2. Desactivar el producto independiente 'Monoesencia 5 ML' (id_product=527)
...
mariadb -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" -e "
SELECT COUNT(*) as combinaciones_desactivadas
FROM ps_product_attribute pa
JOIN ps_product_attribute_combination pac ON pa.id_product_attribute = pac.id_product_attribute
JOIN ps_category_product cp ON pa.id_product = cp.id_product
WHERE pac.id_attribute = 6
AND cp.id_category IN (82, 114)
AND pa.available_for_order = 0;
"

echo ">>> Listo. 44 combinaciones + producto 527 desactivados sin tocar el stock."
