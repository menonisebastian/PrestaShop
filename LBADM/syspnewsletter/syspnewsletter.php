...
class SyspNewsletter extends Module
{
    ...
    public function uninstall()
    {
        ...
        $keys = [
            'SYSPNL_ACTIVE', 'SYSPNL_FREQUENCY', 'SYSPNL_FREQUENCY_VALUE',
            'SYSPNL_DELAY', 'SYSPNL_TITLE', 'SYSPNL_SUBTITLE', 'SYSPNL_BTN_TEXT',
            'SYSPNL_PLACEHOLDER', 'SYSPNL_FONT_FAMILY', 'SYSPNL_FONT_SIZE_TITLE',
            'SYSPNL_FONT_SIZE_SUBTITLE', 'SYSPNL_COLOR_BG', 'SYSPNL_COLOR_OVERLAY',
            'SYSPNL_COLOR_TITLE', 'SYSPNL_COLOR_SUBTITLE', 'SYSPNL_COLOR_BTN_BG',
            'SYSPNL_COLOR_BTN_TEXT', 'SYSPNL_COLOR_INPUT_BORDER', 'SYSPNL_BORDER_RADIUS',
            'SYSPNL_WIDTH', 'SYSPNL_ANIMATION', 'SYSPNL_DISCOUNT_ACTIVE',
            'SYSPNL_DISCOUNT_TYPE', 'SYSPNL_DISCOUNT_VALUE', 'SYSPNL_DISCOUNT_MSG',
            'SYSPNL_SUCCESS_MSG', 'SYSPNL_BG_IMAGE', 'SYSPNL_POSITION',
        ];
        foreach ($keys as $key) {
    ...
    protected function getFormGeneral()
    {
        return [
            'form' => [
                ...
                'input' => [
                    [
                        'type' => 'switch', 'label' => $this->l('Activar popup'),
                        'name' => 'SYSPNL_ACTIVE', 'is_bool' => true,
                        'values' => [
                    ...
                    [
                        'type' => 'select', 'label' => $this->l('Frecuencia'),
                    ...
                    [
                        'type' => 'text', 'label' => $this->l('Días entre apariciones'),
                        'name' => 'SYSPNL_FREQUENCY_VALUE', 'class' => 'fixed-width-sm',
                        'desc' => $this->l('Solo si elegiste "Cada X días"'),
                    ],
                    [
                        'type' => 'text', 'label' => $this->l('Delay (segundos)'),
                        'name' => 'SYSPNL_DELAY', 'class' => 'fixed-width-sm',
                        'desc' => $this->l('Segundos que espera antes de aparecer (0 = inmediato)'),
                    ],
                    [
                        'type' => 'select', 'label' => $this->l('Posición'),
                    ...
                    [
                        'type' => 'select', 'label' => $this->l('Animación'),
        ...
        return [
            'form' => [
                ...
                'input' => [
                    // Textos
                    [
                        'type' => 'text', 'label' => $this->l('Título'),
                        'name' => 'SYSPNL_TITLE',
                        'desc' => $this->l('Título principal del popup'),
                    ],
                    [
                        'type' => 'textarea', 'label' => $this->l('Subtítulo'),
                        'name' => 'SYSPNL_SUBTITLE', 'rows' => 3,
                        'desc' => $this->l('Texto descriptivo debajo del título'),
                    ],
                    [
                        'type' => 'text', 'label' => $this->l('Texto del botón'),
                        'name' => 'SYSPNL_BTN_TEXT',
                    ],
                    [
                        'type' => 'text', 'label' => $this->l('Placeholder del campo email'),
                        'name' => 'SYSPNL_PLACEHOLDER',
                    ],
                    [
                        'type' => 'text', 'label' => $this->l('Mensaje de éxito'),
                        'name' => 'SYSPNL_SUCCESS_MSG',
                        'desc' => $this->l('Se muestra tras suscribirse correctamente'),
                    ],
                    // Dimensiones
                    [
                        'type' => 'text', 'label' => $this->l('Ancho del popup (px)'),
                        'name' => 'SYSPNL_WIDTH', 'class' => 'fixed-width-sm',
                    ],
                    [
                        'type' => 'text', 'label' => $this->l('Radio de bordes (px)'),
                        'name' => 'SYSPNL_BORDER_RADIUS', 'class' => 'fixed-width-sm',
                    ],
                    // Tipografía
                    [
                        'type' => 'select', 'label' => $this->l('Fuente'),
                    ...
                    [
                        'type' => 'text', 'label' => $this->l('Tamaño título (px)'),
                        'name' => 'SYSPNL_FONT_SIZE_TITLE', 'class' => 'fixed-width-sm',
                    ],
                    [
                        'type' => 'text', 'label' => $this->l('Tamaño subtítulo (px)'),
                        'name' => 'SYSPNL_FONT_SIZE_SUBTITLE', 'class' => 'fixed-width-sm',
                    ],
                    // Colores
                    [
                        'type' => 'color', 'label' => $this->l('Color de fondo del popup'),
                        'name' => 'SYSPNL_COLOR_BG',
                    ],
                    [
                        'type' => 'text', 'label' => $this->l('Color del overlay (fondo oscuro)'),
                        'name' => 'SYSPNL_COLOR_OVERLAY',
                        'desc' => $this->l('Formato: rgba(0,0,0,0.65) — puedes ajustar la opacidad'),
                    ],
                    [
                        'type' => 'color', 'label' => $this->l('Color del título'),
                        'name' => 'SYSPNL_COLOR_TITLE',
                    ],
                    [
                        'type' => 'color', 'label' => $this->l('Color del subtítulo'),
                        'name' => 'SYSPNL_COLOR_SUBTITLE',
                    ],
                    [
                        'type' => 'color', 'label' => $this->l('Color del botón (fondo)'),
                        'name' => 'SYSPNL_COLOR_BTN_BG',
                    ],
                    [
                        'type' => 'color', 'label' => $this->l('Color del botón (texto)'),
                        'name' => 'SYSPNL_COLOR_BTN_TEXT',
                    ],
                    [
                        'type' => 'color', 'label' => $this->l('Color borde del input'),
                        'name' => 'SYSPNL_COLOR_INPUT_BORDER',
                    ],
                    [
                        'type' => 'text', 'label' => $this->l('Imagen de fondo (URL o ruta)'),
    ...
    protected function getFormDiscount()
    {
        return [
            'form' => [
                ...
                'input' => [
                    [
                        'type' => 'switch', 'label' => $this->l('Activar descuento'),
                        'name' => 'SYSPNL_DISCOUNT_ACTIVE', 'is_bool' => true,
                    ...
                    [
                        'type' => 'select', 'label' => $this->l('Tipo de descuento'),
                    ...
                    [
                        'type' => 'text', 'label' => $this->l('Valor del descuento'),
                        'name' => 'SYSPNL_DISCOUNT_VALUE', 'class' => 'fixed-width-sm',
                        'desc' => $this->l('Ej: 10 (para 10% o 10€). No aplica para envío gratis.'),
                    ],
                    [
                        'type' => 'text', 'label' => $this->l('Mensaje con el cupón'),
    ...
    {
        $keys = [
            'SYSPNL_ACTIVE', 'SYSPNL_FREQUENCY', 'SYSPNL_FREQUENCY_VALUE', 'SYSPNL_DELAY',
            'SYSPNL_TITLE', 'SYSPNL_SUBTITLE', 'SYSPNL_BTN_TEXT', 'SYSPNL_PLACEHOLDER',
            'SYSPNL_FONT_FAMILY', 'SYSPNL_FONT_SIZE_TITLE', 'SYSPNL_FONT_SIZE_SUBTITLE',
            'SYSPNL_COLOR_BG', 'SYSPNL_COLOR_OVERLAY', 'SYSPNL_COLOR_TITLE',
            'SYSPNL_COLOR_SUBTITLE', 'SYSPNL_COLOR_BTN_BG', 'SYSPNL_COLOR_BTN_TEXT',
            'SYSPNL_COLOR_INPUT_BORDER', 'SYSPNL_BORDER_RADIUS', 'SYSPNL_WIDTH',
            'SYSPNL_ANIMATION', 'SYSPNL_DISCOUNT_ACTIVE', 'SYSPNL_DISCOUNT_TYPE',
            'SYSPNL_DISCOUNT_VALUE', 'SYSPNL_DISCOUNT_MSG', 'SYSPNL_SUCCESS_MSG',
            'SYSPNL_BG_IMAGE', 'SYSPNL_POSITION',
        ];
